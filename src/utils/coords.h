...
struct CoordPair {
  ...
  bool operator<(const CoordPair& c2) const {
    if (x == c2.x) {
      return y > c2.y;
    }
  ...
  bool operator==(const CoordPair& c2) const { return x == c2.x && y == c2.y; }

  CoordPair operator+(const CoordPair& right) const { return CoordPair(x + right.x, y + right.y); }
  CoordPair operator-(const CoordPair& right) const { return CoordPair(x - right.x, y - right.y); }

  CoordPair del(float d) const { return CoordPair(static_cast<T>(x / d), static_cast<T>(y / d)); }
  CoordPair operator/(float d) const { return del(d); }
...
class HashFunctionCoord {
 public:
  size_t operator()(const CoordPair<int>& c) const { return c.x * max_map_size + c.y; }
};
